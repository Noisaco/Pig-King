...
int menu(Tigr *pantalla, int ancho_ventana, int alto_ventana) {
    ...
    int aparicion_botones=1, seleccion=0, click=0, last_click=0, botes=0, elevacion=0, i=0, j=0;
    ...
    TPixel presion_boton_jugar, presion_boton_niveles, presion_boton_puntuacion, si_presionado=tigrRGB(70,83,199),  no_presionado=tigrRGB(116, 130, 252);
    ...
    while(!seleccionado && !tigrClosed(pantalla) && !tigrKeyHeld(pantalla, TK_ESCAPE)) {
        ...
        if((resguardo==(2*alto_ventana/8)) && (velocidad_menu>0) && (botes!=4)) { 
        ...
        if((!opening) && (cursor.x>=boton_jugar.x) && (cursor.y>=boton_jugar.y)&& (cursor.x<=boton_jugar.x+boton_jugar.w) && (cursor.y<=boton_jugar.y+boton_jugar.h)) {
            seleccion=1;
        }
        else if((!opening) && (cursor.x>=boton_niveles.x) && (cursor.y>=boton_niveles.y) && (cursor.x<=boton_niveles.x+boton_niveles.w) && (cursor.y<=boton_niveles.y+boton_niveles.h)) {
            seleccion=2;
        }
        else if((!opening) && (cursor.x>=boton_puntuacion.x) && (cursor.y>=boton_puntuacion.y) && (cursor.x<=boton_puntuacion.x+boton_puntuacion.w) && (cursor.y<=boton_puntuacion.y+boton_puntuacion.h)) {
            seleccion=3;
        }
        else {
            seleccion=0;
        ...
        last_click=click;
        tigrSetPostFX(pantalla, 1, 1, 1, 2.0f); // Inserta el efecto de television de tubos.
        tigrClear(pantalla, tigrRGB(0, 0, 0)); // Limpia la pantalla.
        tigrFill(pantalla, cortina.x, 0, cortina.w, cortina.y, color_azul); // Cortina del menu.
        tigrBlitAlpha(pantalla, sombra.i, sombra.x+(elevacion*0.5), sombra.y, 0, 0, sombra.w, sombra.h, 1);
        tigrBlitAlpha(pantalla, titulo.i, titulo.x, titulo.y-elevacion, 0, 0, titulo.w, titulo.h, 1);
        tigrFill(pantalla, cortina.x, cortina.y, cortina.w, 5, color_morado);
        ...
        tigrFill(pantalla, plantilla.x, plantilla.y, plantilla.w, plantilla.h, color_azul);
        // Maco plantilla.
        tigrFill(pantalla, plantilla.x, plantilla.y, plantilla.w, 3, color_morado);
        tigrFill(pantalla, plantilla.x, plantilla.y, 3, plantilla.h, color_morado);
        tigrFill(pantalla, plantilla.x+plantilla.w-3, plantilla.y, 3, plantilla.h, color_morado);
        tigrFill(pantalla, plantilla.x, plantilla.y+plantilla.h-3, plantilla.w, 3, color_morado);
        // Botones.
        tigrFill(pantalla, boton_jugar.x, boton_jugar.y, boton_jugar.w, boton_jugar.h, presion_boton_jugar);
        tigrFill(pantalla, boton_niveles.x, boton_niveles.y, boton_niveles.w, boton_niveles.h, presion_boton_niveles);
        tigrFill(pantalla, boton_puntuacion.x, boton_puntuacion.y, boton_puntuacion.w, boton_puntuacion.h, presion_boton_puntuacion);
        // Marco boton Jugar.
        tigrFill(pantalla, boton_jugar.x, boton_jugar.y, boton_jugar.w, 2, tema_boton_jugar);
        tigrFill(pantalla, boton_jugar.x, boton_jugar.y, 2, boton_jugar.h, tema_boton_jugar);
        tigrFill(pantalla, boton_jugar.x+boton_jugar.w-2, boton_jugar.y, 2, boton_jugar.h, tema_boton_jugar);
        tigrFill(pantalla, boton_jugar.x, boton_jugar.y+boton_jugar.h-2, boton_jugar.w, 2, tema_boton_jugar);
        // Marco boton Niveles.
        tigrFill(pantalla, boton_niveles.x, boton_niveles.y, boton_niveles.w, 2, tema_boton_niveles);
        tigrFill(pantalla, boton_niveles.x, boton_niveles.y, 2, boton_niveles.h, tema_boton_niveles);
        tigrFill(pantalla, boton_niveles.x+boton_niveles.w-2, boton_niveles.y, 2, boton_niveles.h, tema_boton_niveles);
        tigrFill(pantalla, boton_niveles.x, boton_niveles.y+boton_niveles.h-2, boton_niveles.w, 2, tema_boton_niveles);
        // Marco boton Puntuacion.
        tigrFill(pantalla, boton_puntuacion.x, boton_puntuacion.y, boton_puntuacion.w, 2, tema_boton_puntuacion);
        tigrFill(pantalla, boton_puntuacion.x, boton_puntuacion.y, 2, boton_puntuacion.h, tema_boton_puntuacion);
        tigrFill(pantalla, boton_puntuacion.x+boton_puntuacion.w-2, boton_puntuacion.y, 2, boton_puntuacion.h, tema_boton_puntuacion);
        tigrFill(pantalla, boton_puntuacion.x, boton_puntuacion.y+boton_puntuacion.h-2, boton_puntuacion.w, 2, tema_boton_puntuacion);
        // Textos de los botones.
        if(aparicion_botones>=2) {
            tigrPrint(pantalla, tfont, 7.35*ancho_ventana/16 , 6.85*alto_ventana/16, tema_boton_jugar, "JUGAR", 0);
        }
        if(aparicion_botones>=3) {
            tigrPrint(pantalla, tfont, 7*ancho_ventana/16 , 9.85*alto_ventana/16, tema_boton_niveles, "NIVELES", 0);
        }
        if(opening==false) {
            tigrPrint(pantalla, tfont, 6.6*ancho_ventana/16 , 12.85*alto_ventana/16, tema_boton_puntuacion, "PUNTUACIÓN", 0);
        }
        // Texto de los creditos.
        tigrPrint(pantalla, tfont, 5, alto_ventana-15, tigrRGB(225, 225, 225), "Hecho por: Alexander Veldemar, Danil Laurov y Andres Maciá.", 0);
        // Pinta cursor personalizado.
        tigrBlitAlpha(pantalla, cursor.i, cursor.x, cursor.y, 0, 0, 32, 32, 1);
        // Actualiza la imagen.
        tigrUpdate(pantalla);
    }
    return seleccion;
